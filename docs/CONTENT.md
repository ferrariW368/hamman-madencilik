# HAMMARBLE V2 — Content Truth Registry



Production rule: only VERIFIED items may be presented as confirmed factual content.



## Stones



### Spider

Status: PARTIALLY VERIFIED / DETAILS PENDING



Known for project planning:

\- primary/high-importance HAMMARBLE stone



Need verification:

\- official spelling/trade name

\- active sales status

\- quarry/origin

\- approved description

\- surface photography

\- block photography

\- close-up vein photography

\- technical reports

\- stone–quarry relationship



Do not publish technical values until source documents are verified.



### Antalya Beige

Status: PENDING



Need:

\- official trade name

\- active sales status

\- quarry/origin

\- approved photography

\- technical reports

\- stone–quarry relationship



### Other Stones

Status: PENDING



Need:

\- complete current active portfolio

\- official names

\- active status

\- quarry/origin

\- approved media

\- technical reports where available



## Quarries



### Beyşehir / Konya

Status: PENDING FOR PUBLICATION



Need:

\- official quarry identity

\- exact city/country

\- exact verified location/coordinates

\- operational status

\- associated stones

\- approved real photography/video

\- navigation destination



### Other Quarries

Status: PENDING



Need:

\- complete active quarry list

\- city/country

\- verified coordinates

\- stone relationships

\- approved real media

\- navigation destination



No quarry location may be inferred or invented.



## Projects



### Rize-Artvin Airport

Status: PENDING



Need:

\- exact HAMMARBLE involvement

\- stone supplied

\- project year

\- exact scope

\- approved project photographs

\- publication rights



DO NOT CLAIM that HAMMARBLE supplied all marble/stone unless explicitly verified.



### Other Projects

Status: PENDING



Need:

\- project name

\- location

\- year

\- stone used

\- exact HAMMARBLE scope

\- approved media

\- publication rights



## Events



### Egypt / Cairo

Status: PENDING



Need:

\- official event name

\- exact year/date

\- city/country

\- approved stand/team/customer media

\- publication rights



### Marble İzmir

Status: PENDING



Need:

\- exact participation years/dates

\- approved media

\- publication rights



### Other Events

Status: PENDING



Need verified event identity, location, date and approved media before publication.



## Technical Data



Status: DOCUMENT-ONLY VERIFICATION



Acceptable sources:

\- laboratory reports

\- official technical sheets

\- approved source documents



Rules:

\- do not infer missing values

\- do not estimate values

\- keep test method/standard references only when verified

\- keep source document traceable for every published technical value



## Corporate History



Status: PENDING



Known planning direction:

\- family/generational mining heritage



Need:

\- exact names

\- exact dates

\- verified milestones

\- approved historical media

\- publication rights



Do not publish unsupported claims about:

\- company size

\- influence

\- leadership position

\- networks

\- political connections

\- market dominance



## Contact



Status: PENDING



Need verified:

\- head-office address

\- phone

\- email

\- Sales/Export contact

\- WhatsApp

\- WeChat/QR status

\- quarry contacts where applicable

\- form recipient mailbox



No contact detail may be guessed or copied from stale material without verification.



## Legal



Status: PENDING LEGAL



Need:

\- approved Turkish Privacy/KVKK disclosure

\- approved English equivalent/translation

\- legal basis for personal-data handling

\- required form acknowledgement/consent wording

\- data-controller/contact details to publish

\- retention/deletion wording where applicable

\- cookie/analytics disclosure if applicable



Rule:

AI agents must not present legal wording as approved company policy unless the company/legal source has approved it.



## Material Request Tracking



For each DATA-DEPENDENT content group record:

\- source/owner

\- requested date

\- decision deadline

\- verification status

\- fallback/minimum publication state



No agent may invent a deadline.



## DO NOT CLAIM



\- invented slogans

\- unverified technical properties

\- unverified quarry coordinates

\- exaggerated project scope

\- logistics/shipping as a core HAMMARBLE service without verification

\- unsupported "global leader", "best", "largest", "world-class" style claims

\- unverified certifications

\- unverified dates

\- unverified locations

\- publication rights that have not been confirmed

## Existing Public Website Intake — 2026-09-24

Status: PENDING — current public website is a candidate source only. Its copy, contact data and media must not be treated as VERIFIED without company confirmation.

Source: https://www.hammanmadencilik.com/

Observed candidate information:

\- The public site presents Hamman Madencilik as a marble/natural-stone quarry operator and includes generic block, slab, tile and custom-marble product pages.
\- Spider is not named, and the public site does not provide Spider surface/block media, a verified quarry relationship, technical values or technical reports.
\- No verified quarry identity, coordinates, project scope, event record or corporate-history detail was found for V2 use.
\- The public homepage lists a Beyşehir / Konya address, while its contact page lists a Konyaaltı / Antalya address. Treat both as conflicting candidates; neither may be published in V2 until the company confirms the current address.
\- The same public source displays a phone number and info@hammanmadencilik.com.tr, but both remain PENDING until current sales/export contact details are confirmed.

V2 handling:

\- Do not migrate generic claims, logistics claims, product claims or technical attributes from the existing public site.
\- Do not use its imagery or logo files until provenance and publication permission are recorded in docs/ASSETS.md.
\- Keep V2's block-sales direction and truthful temporary states until verified company material arrives.
